...
class FP16_Optimizer(object):
    ...
    and enable an fp16 spk_embedder to be trained using a master copy of fp32 weights.
    ...
    Args:
        ...
        static_loss_scale (float, optional, default=1.0):  Loss scale used internally to scale fp16 gradients computed by the spk_embedder.  Scaled gradients will be copied to fp32, then downscaled before being applied to the fp32 master params, so static_loss_scale should not affect learning rate.
    ...
    def step(self, closure=None): # could add clip option.
        ...
        another forward pass using their spk_embedder.
    ...
    def backward(self, loss, update_fp32_grads=True):
        ...
        .. note::
            The gradients found in an fp16 spk_embedder's leaves after a call to
        ...
        Args:
            loss:  The loss output by the user's spk_embedder.  loss may be either float or half (but see first Note above).
