...
class Decoder(nn.Module):
    ...
    def parse_decoder_outputs(self, acoustic_outputs, gate_outputs, alignments):
        ...
        # (T_out, B) -> (B, T_out)
        # print(torch.stack(gate_outputs).shape)
        # print(gate_outputs[0].shape)
    ...
    def inference(self, memory, memory_lengths):
        ...
        while True:
            ...
            #     pass
            #     # break
            if len(acoustic_outputs) == torch.min(torch.tensor([self.max_decoder_steps, int(memory_lengths.item())], dtype=torch.int)).item():
                break
...
class Tacotron2(nn.Module):
    ...
    def forward(self, inputs, teacher_forcing=1):
        ...
        # inputs: (B, D, T)
        encoder_outputs = self.encoder(inputs, input_lengths.cpu())
